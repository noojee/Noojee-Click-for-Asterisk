 ...
 * 
 * @param initialising
 *            if true this sequence has been called as part of the initialization sequence.
 * @return
 */
...
var sequence = (function(window, undefined)
{
	function Sequence(jobs, param, initialising)
	{
		...
		/*
		 * this.callback = function() { // debugger; njdebug("sequence", "callback this=" + this); var details = this; njdebug("sequence", "sequence.callback: " +
		 * details.readyState + ":" + details.status +":" + details.statusText +":" + details.responseHeader +":" + details.responseText);
		 * 
		 * if (details.readyState < 4) return;
		 * 
		 * if (details.readyState == 4 && details.status == 200) { njdebug("sequence", "reponse loaded"); this.sequence.load(); } else { njdebug("sequence",
		 * "response error, status=" + details.status); prompts.njAlert(details.responseText); } }
		 */
		this.load = function()
		{
			var details = this;
			...
			if (details.status == 200)
			{
				...
				{
					logging.getInstance().njdebug("sequence", "No response parser found for current job, using default parser");
			...
			{
				util.showError(details.statusText,
			...
			try
			{
				...
				else
				{
					if (details.responseText === null || details.responseText.length === 0)
					{
						...
						if (!initialising)
							util.showError(details.responseText, "Unable to connect to Asterisk. Asterisk may be down or your configuration may be incorrect.");
					}
					else
					{
