...
var job = (function(window, undefined)
{
	...
	function Dial()
	{
		...
		this.run = function(sequence, phoneNo)
		{
			...
			// If no tech specified then add SIP/ otherwise use the selected
			// tech.
			var channel = "&Channel=";
			...
			if (callerID !== null)
			{
				...
				url += "&CallerId=" + normalised; // no caller id so we like
			// to display the number we
			// dialing
			// on the users handset.

			// The next two variables are set to display the number that
			// is being dialed on the local handset's LCD.
			// The CALLERID(Name) is set for sip phones that can display text (I
			// think this might work)

			url += "&Variable=CALLERID(Num)=" + normalised; // Hopefully sets
			// the handsets
			// display
			url += "&Variable=NoojeeAPICallerIDNum=" + callerID; // NoojeePBX
			// over-writes
			// CALLERID(NUM)
			// but will
			// use this.
			url += "&Variable=CALLERID(Name)=" + "Dialing: " + phoneNo; // and
			// this
			// as
			// well.
			url += "&Variable=njClickOriginated=true"; // Tells Noojee Answer
			// and everybody else
			// that NJ click started
			// the call.

			var supressNjAnswer = options.getInstance().getBoolValue("supressNoojeeAnswerBar.enabled");
			if (supressNjAnswer)
				url += "&Variable=njAnswerIgnore=true"; // Tells Noojee
			// Answer to ignore
			// the call. If you
			// want pops on NJ
			// Click originated
			...
			if (result.response != "Success")
			{
				...
				if (result.message == "Authentication Required")
				{
					...
					var sequence = new sequence.Sequence(
						[ new job.Login(), new job.Wait(null) ], "", false);
					sequence.run();
				}
	...
	function Wait(subSequence)
	{
		...
		this.run = function(sequence, param)
		{
			...
			// The first time through we want wait to return immediately so we
			// can
	...
	function HangupAction(channel)
	{
		...
		this.handleResponse = function(result)
		{
			...
			{
				noojeeClick.asterisk.getInstance().updateState('');
			}, 3000);
	...
	function Logoff()
	{
		...
		function genURL(command)
		{
			...
			// The asterisk appliance doesn't allow a prefix so we need to support a
			// null prefix
