	...
	function initializeNewModule()
	{
		...
		function init()
		{
			...
			var sequence = new sequence.Sequence(
				[ new job.Login(), new job.Wait(null) ], "", true);
			sequence.run();
		}
		;
		...
		function dial(phoneNo)
		{
			...
			var dialSequence = new sequence.Sequence(
				[ new job.Dial(), new job.Complete() ], phoneNo, false);
			...
			var loginSequence = new sequence.Sequence(
				[ new job.Login(), new job.Wait(dialSequence) ], "", true);
			loginSequence.run();
		}
		;
		...
		function answer()
		{
			...
			var answerSequence = new sequence.Sequence(
				[ new job.Answer(), new job.Complete() ], this.remoteChannel, false);
			var loginSequence = new sequence.Sequence(
				[ new job.Login(), new job.Wait(answerSequence) ], "", true);
		...
		function hangup()
		{
			...
			var hagupSequence = new sequence.Sequence(
				[ new job.HangupAction(this.channel), new job.Complete() ], "", false);
			var loginSequence = new sequence.Sequence(
				[ new job.Login(), new job.Wait(hagupSequence) ], "", true);
			loginSequence.run();
		...
		{
			var sequence = new sequence.Sequence(
				[ new sequence.Logoff(), new sequence.Complete() ], "", false);
			sequence.run();
		}
		;
