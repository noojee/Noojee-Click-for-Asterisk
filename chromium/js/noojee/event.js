...
eventFactory: function (xmlDoc)
{
	...
	theApp.util.debug("event.low", "eventFactory called");
...
//			else
//				theApp.util.debug("event.low", "Unknown event=" + eventType);
	...
	{
		theApp.util.debug("event.low", events.length + " events found");
		for (var i = 0; i < j; i++)
		{
			theApp.util.debug("event.low", "events[i]=" + events[i].name);
		}
	}
...
NewChannel: function (response)
{
	...
	this.apply = function(asterisk)
	{
		...
		{
			theApp.util.debug("event.high", "NewChannel received for local channel=" + this.channel + " state=" + this.state);
			theApp.util.debug("event.high", "extractChannel=" + theApp.util.extractChannel(this.channel).toLowerCase());
			theApp.util.debug("event.high", "getLocalChannel()=" + theApp.util.getLocalChannel().toLowerCase());
		...
		{
			theApp.util.debug("event.high", "NewChannel received for Remote channel=" + this.channel + " state=" + this.state);
			asterisk.updateState(this.state);
		}
...
Link: function (response)
{
	...
	this.apply = function(asterisk)
	{
		theApp.util.debug("Applying Event=" + this.name + " channel=" + this.channel);
		
		// First check that the event is ours
		if (theApp.util.isLocalChannel(this.channel))
		{
			theApp.util.debug("event.high", "Link event received.");
...
Unlink: function (response)
{
	this.name = "Unlink";
	this.channel = response.getAttribute("channel1");
	theApp.util.debug("channel=" + this.channel);
	...
	this.apply = function(asterisk)
	{
		...
		if (theApp.util.isLocalChannel(this.channel))
		{
			theApp.util.debug("event.hutiligh", "Unlink event received.");
			asterisk.remoteChannel = this.remoteChannel;
...
Hangup: function (response)
{
	...
	this.apply = function(asterisk)
	{
		...
		if (theApp.util.isLocalChannel(this.channel))
		{
			theApp.util.debug("event.high", "Hangup event received for channel=" + this.channel + " cause=" + this.causeText);
...
Newstate: function (response)
{
	...
	this.apply = function(asterisk)
	{
		theApp.util.debug("event.low", "Newstate event received for channel=" + this.channel + " state=" + this.state);

		if (this.channel == asterisk.remoteChannel)
		{
			theApp.util.debug("event.high", "NewState updated asterisk state to " + asterisk.state);
			asterisk.updateState(this.state);
		}
	};
},
...
Newcallerid: function (response)
{
	...
	this.apply = function(asterisk)
	{
		...
		if (this.channel == asterisk.remoteChannel)
		{
			theApp.util.debug("event.high", "Newcallerid event received for channel=" + this.channel + " callerid=" + this.callerid);
