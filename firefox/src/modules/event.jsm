var EXPORTED_SYMBOLS = ["eventFactory"];
...
function eventFactory(xmlDoc)
{
	...
	for ( var i = 0; i < genericElement.length; i++)
	{
		if (genericElement[i] != null && genericElement[i].getAttribute("event") != null)
		{
			...
			// else some event that we don't care about
...
function NewChannel(response)
{
	...
	this.apply = function(asterisk)
	{
		...
		if (isLocalChannel(this.channel))
		{
			...
			if (this.state == "Down" || this.state == "Ringing")
				asterisk.updateState("Ringing: " + getValue("extension"));
			else if (this.state == "Up")
				; // we ignore these as it just means the local channel is up so we leave in ringing state
					// as it will transition to remote ringing in a moment.
			else
		...
		if (asterisk.isRemoteChannel(this.channel))
		{
			...
			if (this.state == "Down" || this.state == "Ringing")
				asterisk.updateState("Ringing: " + asterisk.getRemoteChannelShortName());
			else if (this.state == "Up")
				; // we ignore these as it just means the channel is up which we don't care about
					// as it will transition to connected in a moment.
			else
				asterisk.updateState(this.state);		
		}
	}
...
function Newstate(response)
{
	...
	this.apply = function(asterisk)
	{
		...
		if (isLocalChannel(this.channel))
		{
			njdebug("event.high", "NewState received for local channel=" + this.channel + " state=" + this.state);
			njdebug("event.high", "extractChannel=" + extractChannel(this.channel).toLowerCase());
			njdebug("event.high", "getLocalChannel()=" + getLocalChannel().toLowerCase());
			asterisk.channel = this.channel;
		...
		if (asterisk.isRemoteChannel(this.channel))
		{
			njdebug("event.high", "NewState received for Remote channel=" + this.channel + " state=" + this.state);
